        ...
        `:D`&nbsp;`,k=`${"off"===g?"fader-inactive":"fader-active"}${c?" fader-unavailable":""}`,N="fader_range_"+l.entity_id;let T=`--fader-width: ${e}; --fader-height: ${i}; --fader-border-radius: ${t}; `;T+=`--fader-color: ${"on"===g?x:S}; `,T+=`--fader-thumb-color: ${E}; --fader-track-color: ${w}; --fader-track-inactive-color: ${S};`;const P=Math.round((_-f)/(p-f)*100);let U=D`<input type="range" class="${k}" id="${N}" style="${T}" .value="${P}" @change=${t=>this._setFaderLevel(d,t.target.value)}>`;o&&(U=D`<input type="range" class="${k}" id="${N}" style="${T}" .value="${P}" @input=${t=>this._setFaderLevel(d,t.target.value)}>`),h.push(D`
            <div class = "fader" id = "fader_${l.entity_id}">
              ...
              <div class = "fader-value">${"on"===g||a?b?$t(this.hass.localize,b,this.hass.language):y:D`<br>`}</div>
                <div class = "active-button-holder ${c?"button-disabled":""}">${C}</div>
            </div>
        `)}let u=c?D`<h1 class="card-header"><div class = "name">${c}</div></div>`:"",p=d?D`<p class = "mixer-description">${d}</p>`:"";const f=D`
     ${u}
     ${p}
      <div>
