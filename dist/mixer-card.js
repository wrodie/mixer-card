...
class MixerCard extends s$3 {
  ...
  render() {
    ...
    for (const fader_index in this.config.faders) {
      ...
      let domain = L$1(stateObj);
      if (!['number', 'media_player'].includes(domain)) {
        continue;
      }
      const fader_name = fader_row['name'] || this._entity_property(fader_row.entity_id, this.hass.states, '-name');
      ...
      const activeButton = active_entity ? x`
      ...
      faderTemplates.push(x`
            <div class = "fader" id = "fader_${fader_row.entity_id}">
              ...
              <div class = "fader-value">${activeState === 'on' ? fader_value_state ? W(this.hass.localize, fader_value_state, this.hass.language) : fader_value : x`<br>`}</div>
  ...
  _toggleActive(e) {
    if (e.target.dataset && e.target.dataset.entity) {
      let domain = E$1(e.target.dataset.entity);
      if (domain == "media_player") {
